 ...
 */
	...
	public static void initAI(QActor qa) throws Exception {
		RoomMap.deleteRoomMap();   
 		initialState = new RobotState(0, 0, RobotState.Direction.DOWN); //DOWN
	...
	public static List<Action> doPlan( QActor qa  ) throws Exception {
		...
		if (actions == null || actions.isEmpty()) {
			...
			if( qa != null) qa.emit("mindEvent", "cmd(ias, ia1, offclean)"); //qa.addRule("endOfWork");  
			return null;
		} 
...
//				move + " dir=" + dir +" x=" + x + " y="+y + " dimMapX=" + dimMapx + " dimMapY=" + dimMapy   );
	...
	public static void markGoalCellAsObstacle(QActor qa, String x, String y) {
		int altezza = Integer.parseInt(x);
		int profondita = Integer.parseInt(y);
		RoomMap.getRoomMap().put(qa, altezza, profondita, new Box(true, false, false));
	}
	...
	public static void markCellAsObstacle( QActor qa ) {
		...
		System.out.println("markCellAsObstacle x=" + x + " y=" + y + " dir=" + dir);
		  switch( dir ){
		     case UP     :  RoomMap.getRoomMap().put(qa, x, y-1, new Box(true, false, false)); break;
		     case LEFT   :  RoomMap.getRoomMap().put(qa, x-1, y, new Box(true, false, false)); break;
		     case RIGHT  :  RoomMap.getRoomMap().put(qa, x+1, y, new Box(true, false, false)); break;
		     case DOWN   :  RoomMap.getRoomMap().put(qa, x, y+1, new Box(true, false, false));
		  }		
		
	}
	
	public static void markCellsAsObstacle(QActor qa, String x, String y) {
		int altezza = Integer.parseInt(x);
		int profondita = Integer.parseInt(y);
		for (int i=0; i<=altezza+1;i++) {
		RoomMap.getRoomMap().put(i, profondita+1, new Box(true, false, false));
		RoomMap.getRoomMap().put(i, profondita+2, new Box(true, false, false));
		}
		for (int j=0; j<=profondita+1;j++){
			RoomMap.getRoomMap().put(altezza+1, j, new Box(true, false, false));
			RoomMap.getRoomMap().put(altezza+2, j, new Box(true, false, false));
			
		}
		RoomMap.getRoomMap().put(altezza+2, profondita+2, new Box(true, false, false));
	}
